    ...
    const load = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          // map legacy description if present
    ...
    try {
      ...
      Alert.alert('Delete Forever?', `Delete ${name}?`, [
        ...
        { text: 'Delete', style: 'destructive', onPress: async () => {
          // Match web logic: delete by card name under user's recipes
          await deleteUserDoc('recipes', name);
          Alert.alert('Deleted', `${name} removed`);
          // Go to Home to match requested flow and refresh summary/lists
    ...
    <>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          {/* Instructions */}
          <Text style={[styles.sectionTitle, { color: 'orange' }]}>Instructions</Text>
          <TextInput
            style={[styles.input, styles.multiInput, styles.darkInput]}
            value={instructions}
          ...
          <View style={styles.actionRow}>
            {/* Update this card (only if not NewItem) */}
            {recipeId !== 'NewItem' && (
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                try {
                  ...
                  try { Alert.alert('AI filled', `${g1.length} ingredients lines, ${g2.length} instruction lines`); } catch {}
                } catch (e: any) {
