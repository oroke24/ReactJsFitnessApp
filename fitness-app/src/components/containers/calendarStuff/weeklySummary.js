...
const WeeklySummary = ({ selectedDate, days, onDayClick, email }) => {
    const isLoading = days === null || days === undefined;
            ...
            {isLoading ? (
                Array.from({ length: 7 }).map((_, idx) => (
                    <div key={idx} className="p-1 flex flex-col items-center">
                        {idx === 0 && <h3 className='font-bold text-center text-gray-400 mb-1'>Today</h3>}
                        <div className="w-[380px] h-[300px] bg-gray-200 rounded shadow animate-pulse"></div>
                    </div>
                ))
            ) : (
